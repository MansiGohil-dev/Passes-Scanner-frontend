        ...
        {modalOpen && (
          <div style={{
            ...
            <div style={{ background: '#fff', padding: 32, borderRadius: 8, minWidth: 300, textAlign: 'center', boxShadow: '0 2px 10px #0003' }}>
              {scanResult && scanResult.expired ? (
  <>
    <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 10, color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <span style={{ fontSize: 32, marginRight: 8 }}>❌</span> QR Expired
    </div>
    <div style={{ marginBottom: 12, color: '#dc2626' }}>This QR code has already been scanned.</div>
  </>
) : scanResult ? (
  <>
    <div style={{
      ...
      fontWeight: 700,
      marginBottom: 10,
      color: scanResult.allowed ? '#16a34a' : '#dc2626',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center'
    }}>
      <span style={{ fontSize: 32, marginRight: 8 }}>
        {scanResult.allowed ? '✔️' : '❌'}
      </span>
      {scanResult.allowed ? 'Access Allowed' : 'Access Denied'}
    </div>
    <div style={{ marginBottom: 8 }}><b>User Name:</b> {scanResult.name || 'N/A'}</div>
    {scanResult.message && <div style={{ marginBottom: 8, color: scanResult.allowed ? '#16a34a' : '#dc2626' }}>{scanResult.message}</div>}
  </>
) : (
  <div style={{ fontSize: 20, fontWeight: 600, marginBottom: 12, color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    <span style={{ fontSize: 32, marginRight: 8 }}>❌</span> {modalMessage}
  </div>
)}
              <button
                style={{ padding: '8px 32px', borderRadius: 4, background: '#4F46E5', color: '#fff', border: 'none', fontWeight: 600, fontSize: 16 }}
