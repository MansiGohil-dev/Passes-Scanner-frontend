  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-green-50 to-green-200 p-4">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md text-center">
        ...
        {/* QR Code for this user's pass */}
        <div className="mb-4 flex flex-col items-center">
          <span className="font-semibold text-gray-700 mb-1">Your Pass QR</span>
          <QRCodeSVG
        ...
        {/* Pass Info */}
        <div className="mb-2 text-lg">Name: <span className="font-bold">{info.name || 'N/A'}</span></div>
        <div className="mb-2 text-lg">Mobile: <span className="font-mono text-blue-700">{info.mobile}</span></div>
        <div className="mb-2 text-lg">Total Passes: <span className="font-bold">{info.count}</span></div>
        <div className="mb-2 text-lg">Passes Remaining to Share: <span className="font-bold text-green-700">{info.remaining}</span></div>
        {/* Section for sharing extra passes (if allowed) */}
        {info.remaining > 0 && (
        ...
        <p className="text-xs text-gray-400 mt-4">Show this QR code at the event or to the admin for verification. If you have extra passes, share them below.</p>
      </div>
      {/* OTP Modal */}
      {showOtp && (
